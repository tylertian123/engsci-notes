...
* We can now rewrite $\alignedeqntwo[t]{T}{\sum _{i = 1}^n \frac{1}{2}m_i\dot q_i^TJ_{v_i}^TJ_{v_i}\dot q_i + \frac{1}{2}\dot q_i^TJ_{w_i}^TI_i(q)J_{w_i}\dot q_i}{\frac{1}{2}\dot q^T\left(\sum _{i = 1}^n m_iJ_{v_i}^T(q)J_{v_i}(q) + J_{w_i}^T(q)I_i(q)J_{w_i}^T(q)\right)\dot q}{\frac{1}{2}\dot q^TD(q)\dot q}$
	* $D(q) = \sum _{i = 1}^n m_iJ_{v_i}^T(q)J_{v_i}(q) + J_{w_i}^T(q)I_i(q)J_{w_i}^T(q)$ is a symmetric positive definite matrix
