...
* Let the estimation error $\tilde z_d(k) = \hat z_d(k) - z(k)$, then $\tilde z_d(k + 1) = (A - L_dC)\tilde z_d(k) + Bd(k)$
	* Let $\alignedeqntwo[t]{d_f(k)}{C\tilde z_d(k)}{C\hat z_d(k) - Cz(k)}{C\hat z_d(k) - e(k)}$
		* This means $d_f(k)$ is fully measurable since we compute $\hat z_d(k)$ and we can measure $e(k)$
	...
	* Therefore $d_f(k)$ is generated by $\begin{aligned}[t] w_f(k + 1) &= Fw_f(k) + Gd_f(k) \\ d_f(k) &= \psi^Tw_f(k) \end{aligned}$
		...
		* See lemma in Lecture 32 and the discussion that follows about why this is the case
