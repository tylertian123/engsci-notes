...
\textbf{Derivation:} Get \(\dot{\bm q}(\dot{\bm\varphi})\) from kinematics, discretize to get \(\bm q(t + h)\) and split
into \(\bar{\bm q}(t) + \delta\bm q(t)\). Linearize about \(\bar{\bm q}(t)\), drop H.O.T. and take 
\(\bar{\bm q}(t + h) = \mathbb E[\bm q(t + h)]\). Subtract mean and take \(\bm\Sigma(t + h) = \mathbb E[\delta\bm q(t)\delta\bm q(t)^T]\).
...
\begin{tight}
  \begin{align*}
    ...
    &= \bm A(t)\delta\bm q(t) + \bm B(t)\bm w(t) \\
    \bm\Sigma(t + h) &= \bm A(t)\bm\Sigma(t)\bm A(t)^T + \bm B(t)\bm Q\bm B(t)^T
  \end{align*}
\end{tight}
...
\[\mathcal J = \frac{1}{2}\sum _{j = 1}^J w^j\left(\bm p_M^i - \bm C_k\left(\bm p_D^j - \bm t_k\right)\right)^T{\bm n^j}^T{\bm n^j}\left(\bm p_M^i - \bm C_k\left(\bm p_D^j - \bm t_k\right)\right)\]
...
\textbf{Extended Kalman Filter:} Nonlinear \(\bm x_k = \bm f(\bm x_{k - 1}, \bm v_k, \bm w_k)\), \(\bm y_k = \bm g(\bm x_k, \bm n_k)\):
\begin{enumerate}
  \item Prediction: $\twopiece{\check{\bm P}_k = \bm F_{k - 1}\hat{\bm P}_k\bm F_{k - 1}^T + \bm Q_k'}{\check{\bm x}_k = \bm f(\hat{\bm x}_{k - 1}, \bm v_k, \bm 0)}$,
        \(\displaystyle \bm F_{k - 1} = \pdiff{\bm f(\bm x_{k - 1}, \bm v_k, \bm w_k)}{\bm x_{k - 1}}\).
  \item Kalman gain: $\bm K_k = \check{\bm P}_k\bm G_k^T(\bm G_k\check{\bm P}_k\bm G_k^T + \bm R_k')^{-1}$,
        \(\displaystyle \bm G_k = \pdiff{\bm g(\bm x_k, \bm n_k)}{\bm x_k}\).
