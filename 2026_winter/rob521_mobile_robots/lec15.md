...
* To quantify the uncertainty, we can look at the *negative entropy* (aka *Shannon information*): $H(\bm X) = -E[\ln p(\bm x)] = -\int _a^b p(\bm x)\ln p(\bm x)\,\dd\bm x$
	* Note, $H$ is a function of the random variable $\bm X$ (i.e. its distribution), not a specific value of $\bm x$
	...
	* Therefore the Shannon information for a Gaussian is just $\frac{1}{2}\ln((2\pi e)^N\det\bm\Sigma)$
