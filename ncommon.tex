...
\newcommand{\suchthat}{\enspace\mathrm{s.t.}\enspace}
...
\newcommand{\ztf}[1]{\mathcal{Z}\left\{ #1 \right\}}
\newcommand{\iztf}[1]{\mathcal{Z}^{-1}\left\{ #1 \right\}}
...
\newcommand{\ReN}{\mathrm{Re}} % Reynolds number
\newcommand{\FoN}{\mathrm{Fo}} % Fourier number
\newcommand{\BiN}{\mathrm{Bi}} % Biot number
\newcommand{\NuN}{\mathrm{Nu}} % Nusselt number
\newcommand{\PrN}{\mathrm{Pr}} % Prandtl number
\newcommand{\FrN}{\mathrm{Fr}} % Froude number
...
\newcommand{\matthreeB}[9]{\begin{Bmatrix} {#1} & {#2} & {#3} \\ {#4} & {#5} & {#6} \\ {#7} & {#8} & {#9} \end{Bmatrix}}
% For 3x3 and up, since normal macros cannot take more than 9 arguments, we need to break it up into two parts and use some magic
\newcommand{\matthree}[7][b]{%
  \def\matthreematrixtype{#1}%
  \csname #1matrix\endcsname
  {#2} & {#3} & {#4} \\%
  {#5} & {#6} & {#7} \\%
  \matthreecontinued
}
\newcommand{\matthreecontinued}[3]{%
  {#1} & {#2} & {#3}
  \csname end\matthreematrixtype matrix\endcsname
}

\newcommand{\matfour}[9][b]{%
  \def\matfourmatrixtype{#1}%
  \csname #1matrix\endcsname
  {#2} & {#3} & {#4} & {#5} \\%
  {#6} & {#7} & {#8} & {#9} \\%
  \matfourcontinued
}
\newcommand{\matfourcontinued}[8]{%
  {#1} & {#2} & {#3} & {#4} \\%
  {#5} & {#6} & {#7} & {#8}
  \csname end\matfourmatrixtype matrix\endcsname
}
...
% Shorthand for matrix environment
\newcommand{\mat}[2][b]{\begin{#1matrix} #2 \end{#1matrix}}
% For more complex matrices use this: \mat{\mrow{a}{b}{c} ...}
\makeatletter % Matrix row
\newcommand{\mrow}[1]{#1 \mrowchecknextarg}
\newcommand{\mrowchecknextarg}{\@ifnextchar\bgroup{\mrownextarg}{\\}}
\newcommand{\mrownextarg}[1]{& #1 \@ifnextchar\bgroup{\mrownextarg}{\\}}
\makeatother
...
% Optional argument is alignment option
...
% For inserting text tags at the end of each line
\newcommand{\Tag}{& \qquad &}
